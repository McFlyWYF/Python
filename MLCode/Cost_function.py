...
font = FontProperties(fname = r"E:\PythonProject\MLCode\simsun.ttc",size = 14)
...
def linearRegression(alpha = 0.01,num_iters = 400):
    ...
    plot_X1_X2(X)       #画图看下归一化效果

    X = np.hstack((np.ones((m,1)),X))  # 在X前面加一列1
    print (u"\n执行梯度下降算法....\n")

    theta = np.zeros((col,1))
    y = y.reshape(-1,1)    #将行向量转为列向量
    theta,J_history = gradientDescent(X,y,theta,alpha,num_iters)

    plotJ(J_history,num_iters)

    return mu,sigma,theta           #返回均值mu,标准差signma，和学习的结果theta
...
def featureNormaliza(X):
    ...
    mu = np.mean(X_norm,0) # 求每列的平均值(0为列，1为行)
...
def plotJ(J_history,num_iters):
    ...
    plt.xlabel(u"迭代次数",fontproperties = font)#注意指定字体，不然出现乱码
